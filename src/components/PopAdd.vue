...
export default {
  ...
  setup(props, ctx) {
    ...
    const state = reactive({
      ...
      remark: id ? props.detail.remark : '' // 备注信息
    })
    // 初始化，请求消费类型列表
    onMounted(async () => {
    ...
    const addBill = async () => {
      ...
      if (id) {
        // 有 id 的情况下，说明就是编辑状态，参数需要带上 id 属性。
        params.id = id
        // 如果有 id 需要调用详情更新接口
...
<style lang="less" scoped>
  ...
  /**
   * 代码分析
   * 金额的展示标签，添加了一个 animation 类名，目的是为了实现模拟输入框光标闪烁的效果，css 样式已经写明。
   * van-number-keyboard 的 show 属性要一直设置为 true，的目的就是将其固定在弹窗的底部，
   * 所以在样式中重置 .van-number-keyboard 的 position 属性，否则组件是相对于 body 定位在外面的。
   * 这里注意，不需要再固定弹窗的高度，这里直接把 van-popup 组件的 :style 属性删除。
   */
</style>